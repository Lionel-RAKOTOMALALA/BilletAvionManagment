...
using MySql.Data.MySqlClient; // Ajout de la référence à MySQL

namespace AvionManagment
{
    public partial class Tableau_user : UserControl
    {
        // Liste pour stocker les données des utilisateurs
        private List<UserData> usersList = new List<UserData>();

        // Classe pour représenter les données d'un utilisateur
        ...
        private void InitializeControls()
        {
            ...
            btnAddUser.FlatAppearance.BorderColor = successColor;
        ...
        private void SetupDataGridView()
        {
            ...
            dataGridView.Columns[1].Width = 80; // ID
            dataGridView.Columns[2].Width = 150; // Username
            dataGridView.Columns[3].Width = 150; // Password
        ...
        private void LoadDataFromDatabase()
        {
            try
            {
                ...
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    // Créer une commande SQL pour récupérer les utilisateurs
                    string query = "SELECT id_user, username, password FROM utilisateur";
                    ...
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            // Créer un nouvel objet UserData pour chaque ligne
        ...
        private void LoadData()
        {
            ...
            for (int i = startIndex; i < endIndex && i < usersList.Count; i++)
            {
        ...
        private void EditUser(int userId)
        {
            ...
            UserData user = usersList.FirstOrDefault(u => u.Id == userId);
            if (user != null)
            {
                // Créer une instance du formulaire UpdateUser
                UpdateUser updateUserForm = new UpdateUser
                {
                    UserId = user.Id, // Transmettre l'ID de l'utilisateur
                    Username = user.Username,
                    Password = user.Password
                };

                // Afficher le formulaire en modal
                DialogResult result = updateUserForm.ShowDialog();

                // Si la mise à jour est confirmée (par exemple, après avoir cliqué sur un bouton Enregistrer)
                if (result == DialogResult.OK)
                {
                    // Recharger les données (si nécessaire après mise à jour)
                    LoadDataFromDatabase();
                }
            }
        }
        ...
        private void DeleteUser(int userId)
        {
            ...
            DialogResult result = MessageBox.Show($"Voulez-vous vraiment supprimer cet utilisateur?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private bool DeleteUserFromDatabase(int userId)
        {
            try
            {
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    {
                        MessageBox.Show("Aucun utilisateur n'a été supprimé.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return false;
                    }
                }
            }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dataGridView.Rows)
            {
                ...
                {
                    int userId = Convert.ToInt32(row.Cells[1].Value);
            ...
            {
                DialogResult result = MessageBox.Show($"Voulez-vous vraiment supprimer {selectedIds.Count} utilisateur(s)?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            {
                MessageBox.Show("Veuillez sélectionner au moins un utilisateur à supprimer.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }

        private void btnAddUser_Click(object sender, EventArgs e)
        {
            // Créer une instance du formulaire AjoutUser
            AjoutUser addUserForm = new AjoutUser();

            // Afficher le formulaire en tant que modal
        ...
        protected override void OnResize(EventArgs e)
        {
            ...
            if (panel1 != null && btnDelete != null && btnAddUser != null)
            {
                btnDelete.Location = new Point(this.Width - 230, 15);
                btnAddUser.Location = new Point(this.Width - 120, 15);
            }
